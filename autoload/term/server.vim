...
let s:flag = 0
function! term#server#connect() abort
    "start server one time
    "no need to start server in gvim
    if s:flag == 0 && !term#env#IsGui()
        ...
        if term#env#IsNvim() != 0
            ...
            endtry
            if !term#env#Executable('nvr')
                let $VIM_REMOTE=''
            else
                let $VIM_REMOTE='nvr --servername /tmp/nvim_server '
            endif
        elseif has('clientserver') && exists('*remote_startserver') && term#env#IsDisplay()
            let $IN_VIM='vim'
    ...
    elseif term#env#IsGui()
        let $IN_VIM='gvim'
        let $VIM_REMOTE='gvim '
    endif
endfunction

function! term#server#list() abort
    if has('nvim')
